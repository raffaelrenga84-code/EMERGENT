...
// Lista paesi minima; estendi se vuoi
const COUNTRIES = [
...
function PhoneStep({ onSent, t, lang, setLang }) {
  ...
  const buildFullPhone = (rawInput) => {
    ...
    if (!raw) return null;
    // Se l'utente ha incollato un numero internazionale con +, puliscilo e usalo così com'è
    if (raw.startsWith('+')) {
  ...
  const handleSend = async () => {
    ...
    // Basic length check (dopo prefisso): almeno 7 cifre totali (es. prefisso + numero)
    ...
    setLoading(true);
    try {
      // Supabase invia OTP tramite provider configurato (es. Twilio Verify)
    ...
    <>
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        <select
          ...
          style={{ width: '100%', padding: '10px 12px', borderRadius: 12, border: '1.5px solid var(--sm)', background: 'var(--ab)', fontSize: 14, color: 'var(--k)' }}
        >
          {COUNTRIES.map((c) => (
...
function OtpStep({ phone, onBack, t }) {
  ...
  const verify = async () => {
    ...
    try {
      // Nota: Supabase gestisce la sessione dopo la verifica se usi il suo provider.
      // Se usi Twilio custom, chiama qui il tuo endpoint /api/twilio/verify e poi setSession.
      const { error: err } = await supabase.auth.verifyOtp?.({ phone, token, type: 'sms' }) || { error: null };
      setLoading(false);
      if (err) setError(err.message);
      // Se la sessione è stata creata, Supabase aggiornerà lo stato auth automaticamente.
    } catch (e) {
      setLoading(false);
...
export default function LoginScreen() {
  ...
  // Manteniamo Google login per i test
