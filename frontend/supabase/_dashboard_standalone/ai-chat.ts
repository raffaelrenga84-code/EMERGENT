...
function buildSystemPrompt(lang: string, ctx: FamilyContext | undefined, today: string): string {
  ...
  return [
    ...
    `  [[ACTION:create_task|{"title":"...","category":"care|home|health|admin|spese|other","due_date":"YYYY-MM-DD or null","due_time":"HH:MM or null","location":"... or null"}]]`,
    ...
    `  • Tasks have due_date (YYYY-MM-DD) + optional due_time (HH:MM) + optional location.`,
    `  • Events have starts_at (date + optional time, default 19:00) + optional location.`,
    `  • Extract location when the user mentions a place ("dal panificio", "all'ospedale Civile", "in farmacia").`,
    `  • Extract due_time when the user mentions a time ("alle 16:30", "stasera alle 8").`,
